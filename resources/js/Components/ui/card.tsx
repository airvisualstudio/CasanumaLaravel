...
>(({ className, size = "default", ...props }, ref) => {
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col overflow-hidden rounded-4xl border border-border bg-card py-(--card-spacing) text-sm text-card-foreground transition-shadow hover:shadow-xs [--card-spacing:--spacing(6)] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] *:[img:first-child]:rounded-t-4xl *:[img:last-child]:rounded-b-4xl",
        className
      )}
      {...props}
    />
